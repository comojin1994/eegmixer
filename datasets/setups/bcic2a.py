...
BASE_PATH = f"{WORKING_DIR}/../Datasets/BCI_Competition_IV/BCI_Competition_IV_2a"
os.makedirs(BASE_PATH, exist_ok=True)
SAVE_PATH = (
    f"{WORKING_DIR}/../Datasets/BCI_Competition_IV/BCI_Competition_IV_2a_preprocessed"
)
os.makedirs(SAVE_PATH, exist_ok=True)
DB_PATH = f"{WORKING_DIR}/databases/bcic2a.db"
...
for idx, filename in enumerate(pbar):
    ...
    try:
        ...
        # Bandpass filtering
        raw.filter(
            bandpass_filter[0], bandpass_filter[1], fir_design="firwin", verbose=False
        )
        ...
        # Scaling: 1e6
        epochs_data = epochs.get_data(copy=True) * scaling_factor
        ...
        for i in range(preprocessed_data.shape[0]):
            ...
            cur.execute(
                "INSERT INTO MetaData (Sub, Test, Path) Values(:Sub, :Test, :Path)",
                {
