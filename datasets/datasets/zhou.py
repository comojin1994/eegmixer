...
class Zhou(torch.utils.data.Dataset):
    num_subjects = 9
    ...
    def __init__(self, args, target_subject, is_test=False, transform=None):
        ...
        db_path = os.path.join(args.DB_PATH, "zhou.db")
        ...
        self.pbs_score = (
            load_pbs_scores(args.model, "zhou") if args.litmodel == "pbs" else None
        )
    ...
    def _build_query(self, target_subject, is_test):
        if self.mode == "cls":
            if self.is_subject_independent:
                ...
                raise NotImplementedError("Subject dependent mode is not supported yet")
